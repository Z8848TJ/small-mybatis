...
import com.zzh.mybatis.datasource.unpooled.UnpooledDataSource;
...
/**
 * @author: zzh
 * @description: 有连接池的数据源，与无连接池的数据源相比，需要维护池内信息
 */
        ...
        synchronized (state) {
            ...
            if (connection.isValid()) {
                ...
                if (state.idleConnections.size() < poolMaximumIdleConnections && connection.getConnectionTypeCode() == expectedConnectionTypeCode) {
                    ...
                    // 如果没有手动调用 commit，则会回滚到初始状态，如果手动调用了 commit, 回滚到 commit 后的状态
